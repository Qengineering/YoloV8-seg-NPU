...
int post_process(rknn_app_context_t *app_ctx, rknn_output *outputs, float conf_threshold, float nms_threshold, float scale_w, float scale_h, object_detect_result_list *od_results)
{
    ...
    for (int i = 0; i < boxes_num; i++)
    {
        ...
        // get real box
        od_results->results[i].box.left = box_reverse(od_results->results[i].box.left, model_in_width, 0, scale_w);
        od_results->results[i].box.top = box_reverse(od_results->results[i].box.top, model_in_height, 0, scale_h);
        od_results->results[i].box.right = box_reverse(od_results->results[i].box.right, model_in_width, 0, scale_w);
        od_results->results[i].box.bottom = box_reverse(od_results->results[i].box.bottom, model_in_height, 0, scale_h);
    }
    ...
    // get real mask
    int cropped_height = model_in_height;
    int cropped_width = model_in_width;
